(function () {
    var cr_StarCtrl = function ($rootScope, $scope, $timeout, $element, $safeApply, cr_Enums, cr_CommandsManager, cr_UtilsManager, cr_EventsManager, cr_DrawFactory) {
        ...
        var numPoints_watcher;
        var innerRadius_watcher;
        var outerRadius_watcher;
        var radius_watcher;
        ...
        function attachWatchers() {
            ...
            numPoints_watcher = $scope.$watch('numPoints', function (value) {
                value = parseFloat(value);
                $scope.currentObject.numPoints = value-0;
                $scope.currentTarget.set('numPoints', value-0);
                $scope.currentTarget.setCoords();
            });

            innerRadius_watcher = $scope.$watch('innerRadius', function (value) {
                value = parseFloat(value);
                $scope.currentObject.innerRadius = value-0;
                $scope.currentTarget.set('innerRadius', value-0);
                $scope.currentTarget.setCoords();
            });

            outerRadius_watcher = $scope.$watch('outerRadius', function (value) {
                value = parseFloat(value);
                $scope.currentObject.outerRadius = value-0;
                $scope.currentTarget.set('outerRadius', value-0);
                $scope.currentTarget.setCoords();
            });

            radius_watcher = $scope.$watch('radius', function (value) {
                value = parseFloat(value);
                $scope.currentObject.radius = value-0;
                $scope.currentTarget.set('radius', value-0);
                $scope.currentTarget.setCoords();
            });
        ...
        function detachWatchers() {
            ...
            numPoints_watcher && numPoints_watcher();
            innerRadius_watcher && innerRadius_watcher();
            outerRadius_watcher && outerRadius_watcher();
            radius_watcher && radius_watcher();
        }
        ...
        function fill(obj) {
            ...
            $scope.numPoints = obj.numPoints;
            $scope.innerRadius = obj.innerRadius;
            $scope.outerRadius = obj.outerRadius;
            $scope.radius = obj.radius;
            //
    ...
    angular.module('cr_Customify').controller('cr_StarCtrl', cr_StarCtrl);
})();
